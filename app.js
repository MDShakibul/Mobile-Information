...
const displayPhone = (phones) => {
  ...
  if (limit_Phones.length === 0) {
    document.getElementById("error_message").innerText =
      "This Phone of brand are not available";
...
const displayPhoneDetails = (phoneDetails) => {
  const singlePhone = document.getElementById("phone-info");
  document.getElementById("text_single_phone_details").innerText =
    "Single Mobile Phone Details";
  singlePhone.textContent = "";
  const div = document.createElement("div");
  div.classList.add("col");
  if (!phoneDetails.others) {
    document.getElementById("error_single_phone_details").innerText =
      "This Phone have some feature missing";
    document.getElementById("text_single_phone_details").innerText = "";
  } else {
    document.getElementById("error_single_phone_details").innerText = "";
    div.innerHTML = `
    <div class="card h-100 border border-success border-3 rounded-3">
        ...
        <div class="card-body">
          <h3>Phone Name: <span class="fs-5">${
            phoneDetails.name
          }</span></h3><hr>

          
          <h3>Brand Name: <span class="fs-5">${
            phoneDetails.brand
          }</span></h3><hr>
          <h3>Release Date: <span class="fs-5">${
            phoneDetails.releaseDate
              ? phoneDetails.releaseDate
              : "Release Not Yet"
          }</span></h3><hr>
          <h3>Storage: <span class="fs-5">${
            phoneDetails.mainFeatures.storage
          }</span></h3><hr>
          <h3>ChipSe: <span class="fs-5">${
            phoneDetails.mainFeatures.chipSet
              ? phoneDetails.mainFeatures.chipSet
              : "Not Available"
          }</span></h3><hr>
          <h3>DisplaySize: <span class="fs-5">${
            phoneDetails.mainFeatures.displaySize
          }</span></h3><hr>
          
          <h3>MainFeatures: <span class="fs-5">${
          ...
          <h3> Other: <span class="fs-5">Wlan: ${
            phoneDetails.others.WLAN
              ? phoneDetails.others.WLAN
              : "Not Available"
          },  Bluetooth: ${phoneDetails.others.Bluetooth}, GPS: ${
      phoneDetails.others.GPS
    }, USB: ${phoneDetails.others.USB}</span></h3>
        </div>
      </div>`;
